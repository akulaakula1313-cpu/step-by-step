...
io.on('connection', (socket) => {
    ...
    socket.on('play_with_bots', (maxPlayers) => {
        try {
            ...
            console.error("Ошибка play_with_bots:", e);
        }
    });
    ...
    socket.on('join_room', (code) => {
        try {
            ...
            console.error("Ошибка join_room:", e);
        }
    });
    ...
    socket.on('vote_rematch', (roomCode) => {
        try {
            ...
            io.to(room.id).emit('rematch_voted', {
                votesCount: room.rematchVotes.size,
                totalNeeded: humanPlayers.length
            });
            ...
            console.error("Ошибка vote_rematch:", e);
        }
    });
    ...
    socket.on('player_action', ({ roomCode, action, cardIdx }) => {
        try {
            ...
            if (action === 'play_card') {
                handlePlayCard(room, socket.id, cardIdx);
            } else if (action === 'take') {
                handleTake(room, socket.id);
            } else if (action === 'done') {
                handleDone(room, socket.id);
            }
            scheduleBotTurn(room);
        } catch (e) {
            console.error("Ошибка player_action:", e);
        }
    });
    ...
    socket.on('disconnect', () => {
        try {
            ...
            console.error("Ошибка disconnect:", e);
        }
    });
});
...
function handlePlayCard(room, pId, cardIdx) {
    ...
    if (!hand || cardIdx < 0 || cardIdx >= hand.length) return false;
    ...
    let isAttackerParty = false;
    if (state.table.length === 0) {
        isAttackerParty = (pId === attackerId);
    } else {
        isAttackerParty = (pId !== defenderId);
    }

    if (isAttackerParty) {
        if (state.table.length > 0) {
            let tRanks = getTableRanks(state.table);
            if (!tRanks.has(card.rank)) return false;
            let defHandLen = state.hands[defenderId].length;
            if (state.table.length >= Math.min(6, defHandLen + countDefendedPairs(state.table))) return false;
        }
        ...
        if (uncoveredIdx === -1) return false;
        let attCard = state.table[uncoveredIdx].attack;
        if (!canBeat(attCard, card, state.trumpSuit)) return false;
        hand.splice(cardIdx, 1);
...
function handleTake(room, pId) {
    let state = room.state;
    let defenderId = state.playersInfo[state.defenderIdx].id;
    if (pId !== defenderId || state.table.length === 0) return false;
...
function handleDone(room, pId) {
    let state = room.state;
    let attackerId = state.playersInfo[state.attackerIdx].id;
    if (state.table.length === 0 || !state.table.every(p => p.defense !== null)) return false;
    state.table = [];
    refillAllHands(state);
    if (checkGameOver(room)) { broadcastState(room); return true; }
...
function refillAllHands(state) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        let pObj = state.playersInfo[idx];
        if (!pObj) continue;
        let pId = pObj.id;
    ...
    if (defP.isBot) {
        ...
        if (uncoveredIdx !== -1) {
            ...
            for (let i = 0; i < botHand.length; i++) {
                if (canBeat(attCard, botHand[i], state.trumpSuit)) {
                    bestCardIdx = i;
                    break;
                }
            }
...
function broadcastState(room) {
    io.to(room.id).emit('game_update', room.state);
}
